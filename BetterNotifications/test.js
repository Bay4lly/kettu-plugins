...
(()=>{const env=makeEnv();env.store("UserStore",{getCurrentUser:()=>({id:"me"})});const native=env.add({showNotification(x){return "shown"}});const flux=env.add({subscribe(){},dispatch(a){return a}});const p=load(env);p.onLoad();env.storage.suppressEveryone=true;let r=p.__test.evaluate({id:"1",channel_id:"c",guild_id:"g",content:"@everyone hi",author:{id:"u"}});assert(r.suppress);env.storage.suppressEveryone=false;env.storage.repeatRules="acil=10";r=p.__test.evaluate({id:"2",channel_id:"c",guild_id:"g",content:"ACIL yardım",author:{id:"u"}});assert.strictEqual(r.repeat,10);env.storage.mutedUsers="vip";env.storage.priorityUsers="vip";r=p.__test.evaluate({id:"3",channel_id:"c",content:"x",author:{id:"vip"}});assert.strictEqual(r.suppress,false);env.storage.suppressEveryone=true;const n=native.showNotification({message:{id:"4",channel_id:"c",content:"@everyone x",author:{id:"u"}}});assert.strictEqual(n,null);p.onUnload();console.log("PASS BetterNotifications");})();
