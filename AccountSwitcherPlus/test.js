...
(async()=>{const env=makeEnv();const token="abc.def.abcdefghijklmnopqrstuvwxyz1234567890";env.add({getToken:()=>token});let switched=null;env.add({login(){},logout(){},async switchAccountToken(t){switched=t}});env.store("UserStore",{getCurrentUser:()=>({id:"1",username:"one",globalName:"One",avatar:"a"})});env.ctx.setTimeout=()=>1;const p=load(env);p.onLoad();assert(p.__test.hasCrypto());await p.__test.setPin("1234");const blob=await p.__test.encToken(token);assert(blob.startsWith("v1:"));assert.strictEqual(await p.__test.decToken(blob),token);await p.__test.saveCurrent("main");assert.strictEqual(env.storage.accounts.length,1);await p.__test.switchTo(env.storage.accounts[0]);assert.strictEqual(switched,token);let bad=false;try{await p.__test.unlock("9999")}catch{bad=true}assert(bad);p.onUnload();console.log("PASS AccountSwitcherPlus");})().catch(e=>{console.error(e);process.exit(1)});
